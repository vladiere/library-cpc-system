<template>
  <div class="q-ml-sm q-mt-md">
    <span :class="$q.platform.is.mobile ? 'text-h6 q-mb-md' : 'text-h5 q-mb-md'" style="text-decoration: underline">Books Recommended base in your search </span>
    <div v-if="recommendBooks.length === 0" class="column content-center items-center q-mt-md">
      <q-img :src="ManEmpty" style="width: 15rem;"/>
        <span class="text-grey-9 text-h6">Empty Recommendations</span>
    </div>
    <q-carousel
      v-else
      v-model="activeIndex"
    ...
    >
      ...
      >
        <div class="rounded-borders column full-height items-center justify-center text-capitalize">
          <q-img v-for="bookRecommend in slideImages" :src="bookRecommend.imageUrl" class="rounded-borders q-mb-md cursor-pointer" width="20%" :key="bookRecommend.id" @click="navigateToBookInfo">
          <div class="absolute-bottom text-subtitle1 text-center column content-center">
            <span class="text-subtitle1 text-grey-2">{{ bookRecommend.title }}</span>
            <q-btn :label="bookRecommend.book_stat" color="blue-grey-8" text-color="text-grey-2" padding="5px 10px" dense />
          </div>
          </q-img>
        </div>
      </q-carousel-slide>
