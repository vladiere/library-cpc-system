...
export const handleImagesComparison = async (file: any, role: string) => {
  try {

    // Load the user ID image and enhance its quality
    const userIdImage = await Jimp.read(file);
    ...
    if (text) {
      ...
      if (roleIndex !== -1) {
        ...
        if (role.toUpperCase() === 'STUDENT') {
          ...
          return { message: 'Choose correct role' };
        }
      } else {
        return { message: 'Invalid ID or Select your role'}
      }
    } else {
      console.log('No text detected in the user ID image.');
      return {  message: 'Invalid ID image' };
    }
  } catch (error) {
